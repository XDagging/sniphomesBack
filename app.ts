// @ts-nocheck — Non-call routes use loose typing. Call/WebSocket sections are typed via imports.
...
const DemoSchema = new mongoose.Schema({
    ...
    phoneNumberHash: { type: String, index: true }
})
...
const UserSchema = new mongoose.Schema({
    ...
    operatingArea: { type: Array, required: true },
...
const ThreadSchema = new mongoose.Schema({
    uuid: { type: String, unique: false },
    messageId: { type: String, unique: false },
    threadId: { type: String, unique: true },
    sender: { type: String, unique: false },
    receiver: { type: String, unique: false },
    callFeature: { type: Boolean, unique: false },
    area: { type: String, unique: false },
    action: { type: String, unique: false },
    transcript: { type: Array, unique: false }
})
...
app.post("/webhook", async (req, res) => {
    ...
    switch (eventType) {
        case 'checkout.session.completed': {
            const products = { 1999: 2000, 3499: 6000, 9999: 18000 };
        ...
        case 'customer.subscription.deleted': {
            ...
            user.subscription = { active: false, renewalDate: user.subscription.renewalDate };
            await user.save();
            break;
        }
            ...
            if (users) {
                users.map((user, i) => {
                    ...
                    User.findOneAndUpdate({ uuid: user.uuid }, { campaigns: currentCampaigns }).then((_, err) => {
                        if (err) { console.log(err) } else { console.log("wsg") }
                    })
                })
            }
        }
...
app.post("/sendVerify", (req, res) => {
    ...
    Code.findOne({ emailHash: md5(email) }).then((code, err) => {
        if (err) {
            console.log(err)
            res.status(500).send(JSON.stringify({ code: "err", message: "internal server error" }))
        } else {
            let codeHash = generateCode(8)
            const newCode = new Code({ emailHash: md5(email), code: codeHash })
            if (code) { codeHash = code.code }
            ...
            newCode.save().then(() => {
                ...
                res.status(200).send(JSON.stringify({ code: "ok", message: "code sent" }))
            }).catch((err) => {
                console.log(err)
                res.status(500).send(JSON.stringify({ code: "err", message: "internal server error" }))
            })
...
const dynamicCalls: Record<string, Call> = {};

function callSomeone(phoneNumber: string, uuid: string): void {
    client.calls.create({
        ...
        try {
            ...
            if (parsedMsg.event === "start" && parsedMsg.start && parsedMsg.start.callSid) {
                ...
                if (callInstance) {
                    ...
                    const phoneNum = idToPhone.get(callSid);
...
app.post("/register", (req, res) => {
    ...
    User.findOne({ emailHash: md5(email) }).then((user, err) => {
        if (err) {
            ...
            if (user !== null) {
                res.status(400).send(JSON.stringify({ code: "err", message: "user already exists" }))
            } else {
                Code.findOne({ emailHash: md5(email) }).then((codeVer, err) => {
                    if (err) {
                        ...
                        if (codeVer?.code === code) {
                            bcrypt.hash(password, saltRounds, (err, hash) => {
                                if (err) {
                                    ...
                                    const newUser = new User({
                                        ...
                                        operatingArea: [],
                                        customerId: "",
                                        aiSettings: { name: "Bob", thresholdValue: 0.7, callFeature: false },
                                        dashboardStats: { textsSent: 0, callsMade: 0, leadsGenerated: 0 },
                                        subscription: { active: false, renewalDate: 0 },
                                    })
                                    req.session.user = userId
                                    newUser.save().then(() => {
                                        res.status(200).send(JSON.stringify({ code: "ok", message: "user created" }))
                                    }).catch((err) => {
                                        console.log(err)
                                        res.status(500).send(JSON.stringify({ code: "err", message: "internal server error" }))
                                    })
                            ...
                            res.status(403).send(JSON.stringify({ code: "err", message: "invalid code" }))
                        }
                    }
                })
            }
                ...
                bcrypt.compare(password, user.password, (err, result) => {
                    if (err) {
                        ...
                        if (result) {
                            ...
                            res.status(200).send(JSON.stringify({ code: "ok", message: "login successful" }))
                        } else {
                            res.status(403).send(JSON.stringify({ code: "err", message: "invalid password" }))
                        }
                    }
                })
            } else {
                res.status(400).send(JSON.stringify({ code: "err", message: "user does not exist" }))
            }
        }
    })
})
...
app.get("/getUser", (req, res) => {
    authenticateUser(req).then((id) => {
        if (id === "No user found") {
            res.status(403).send(JSON.stringify({ code: "err", message: "user not found" }))
        } else {
...
app.get("/getLeads", async (req, res) => {
    authenticateUser(req).then((id) => {
        if (id === "No user found") {
            res.status(403).send(JSON.stringify({ code: "err", message: "invalid request" }))
        } else {
            Lead.find({ uuid: id }).then((leads, err) => {
                if (err) {
                    ...
                    if (process.env.NODE_ENV === "DEV") {
                        ...
                        leads = [{
                            uuid: id, date: 1735361710711, new: true, area: 'White County',
                            leadDetails: 'Anthony expressed interest in selling his home.',
                            transcript: [], phoneNumber: '301-272-7224', action: 'sell',
                        }]
                    }
                    res.status(200).send(JSON.stringify({ code: "ok", message: leads }))
                }
            })
        }
    })
        ...
        authenticateUser(req).then((id) => {
            if (id === "No user found") {
                ...
                Lead.findOneAndUpdate({ threadId: threadId }, { new: false }).then(() => {
                    res.status(200).send(JSON.stringify({ code: "ok", message: "success" }))
                })
            }
        })
    } catch (e) {
...
app.post("/updateUser", (req, res) => {
    ...
    try {
        if (!operatingArea || !aiName) throw new Error("yur")
    } catch (e) {
        console.log(e)
        res.status(400).send(JSON.stringify({ code: "err", message: "invalid request" }))
        return
    ...
    if (aiName.length > 0) {
        authenticateUser(req).then((id) => {
            if (id === "No user found") {
                ...
                User.findOne({ uuid: id }).then((user, err) => {
                    if (err) {
                        res.status(500).send(JSON.stringify({ code: "err", message: "server error" }))
                    } else {
                        if (user !== null) {
...
app.post("/startCampaign", (req, res) => {
    ...
    authenticateUser(req).then((id) => {
        if (id === "No user found") {
            res.status(403).send(JSON.stringify({ code: "err", message: "not logged in" }))
        } else {
            User.findOne({ uuid: id }).then((user, err) => {
                if (err) {
                    res.status(500).send(JSON.stringify({ code: "err", message: "internal server error" }))
                    ...
                    if (user !== null) {
                        if (user.operatingArea.length === 0) {
                            res.status(403).send(JSON.stringify({ code: "err", message: "add operating area" }))
                            return
                        }
                        if (user.credits < credits) {
                            res.status(403).send(JSON.stringify({ code: "err", message: "insufficient credits" }))
                        } else {
                            const newCampaign = {
                                target, credits, aiThreshold, address, leads: [],
                                areaTarget: user.operatingArea, date: Date.now(), active: true,
                            }
                            User.findOneAndUpdate({ uuid: user.uuid }, { campaigns: [...user.campaigns, newCampaign], credits: user.credits - credits }).then(() => {
                                ...
                                res.status(200).send(JSON.stringify({ code: "ok", message: "campaign started" }))
                            })
                        }
                    } else {
                        res.status(400).send(JSON.stringify({ code: "err", message: "invalid request" }))
            ...
            if (err) {
                ...
                if (user !== null) {
                    if (user.forgotCode === (null || undefined)) {
                        ...
                        bcrypt.hash(password, saltRounds, (err, hash) => {
                            if (err) {
                                res.status(500).send(JSON.stringify({ code: "err", message: "interval server error" }))
                            } else {
                                User.findOneAndUpdate({ uuid: user.uuid }, { password: hash, forgotCode: null }).then(() => {
                                    res.status(200).send(JSON.stringify({ code: "ok", message: "password updated" }))
                                })
                            }
                        })
                    } else {
        ...
        Demo.findOne({ phoneNumberHash: md5(phoneNumber.trim()) }).then((user, err) => {
            if (err) {
                ...
                if (user === null || (email.toLowerCase() === cmod.decrypt(user.email).toLowerCase())) {
                    ...
                    callSomeone(phoneNumber, "demo")
                } else {
                    res.status(403).send(JSON.stringify({ code: "err", message: "already demoed" }))
                }
            }
        })
    } else {
                ...
                if (err) {
                    ...
                    if (thread) {
                        if (thread.callFeature) {
                            const newLead = new Lead({ uuid: "", date: Date.now() })
                        } else {
...
app.post("/internalEmail", (req, res) => {
    ...
    if (internalCredential === process.env.RECEIVE_CREDENTIAL) {
            ...
            if (message || sender || receiver || subject || originalMessageId) {
                ...
                Thread.findOne({ messageId: queryOriginalId }).then(async (val, err) => {
                    if (err) {
                        ...
                        const fullTranscript = [...val.transcript, { date: Date.now(), sender: sender, message: newMessage }]
                                        ...
                                        processLeadConversion(val.messageId, response.transcript, response.phoneNumber).then(() => {
                                            User.findOne({ uuid: val.uuid }).then(async (user, err) => {
                                                if (err) {
                                                    ...
                                                    if (user) {
                                                        let currentCase = "";
                                                        if (val.action.toLowerCase() === "sell") { currentCase = "Selling" }
                                                        else { currentCase = "Buying" }
                                                        const previousDashboard = user.dashboardStats;
                                                        previousDashboard.leadsGenerated += 1;
                                                        user.dashboardStats = previousDashboard
                                                        await user.save();
                                                        res.status(200).send(JSON.stringify({ code: "ok", message: "all went well" }))
                                                    } else {
                                                        res.status(403).send(JSON.stringify({ code: "err", message: "user not found" }))
                                                    }
                                                }
        ...
        res.status(404).send(JSON.stringify({ code: "err", message: "invalid password or not found" }))
    }
})
            ...
            User.findOne({ uuid: uuid }).then(async (user, err) => {
                if (err) {
                    ...
                    if (user) {
                        ...
                        processOutreach(data, senderEmail, user.aiSettings.name, area, message, subject).then(async (messageData) => {
                            ...
                            idList.map((id, i) => {
                                ...
                                const newThread = new Thread({
                                    ...
                                    transcript: [{ date: Date.now(), sender: senderEmail, message: originalMessage.replace(/(<([^>]+)>)/ig, '').replace("/") }],
                                })
                                newThread.save()
                            })
                        ...
                        res.status(403).send(JSON.stringify({ code: "err", message: "invalid uuid" }))
                    }
                }
            })
        } else {
            res.status(404).send(JSON.stringify({ code: "err", message: "not found" }))
            return
        }
    } catch (e) {
        console.log(e)
...
app.get("/addBlocklist/:id", (req, res) => {
    ...
    try {
        ...
        reportError(e)
        res.status(500).send(JSON.stringify({ code: "err", message: "invalid request" }))
    }
})
            ...
            if (err) {
                if (email.substring(0, 1).toLowerCase() !== email.substring(0, 1).toUpperCase()) {
                    fs.writeFile(fileName, email.toLowerCase(), (err) => {
                        if (err) { reportError(err); console.log(err); }
                        resolve("success")
                    })
                } else {
                    resolve("invalid")
                ...
                fs.appendFile(fileName, `,${email.toLowerCase()}`, (err) => {
                    if (err) { console.log(err); reportError(err); console.log(err) }
                    resolve("success")
                })
            }
        })
