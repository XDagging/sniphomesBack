...
export class Call {
  ...
  constructor(callSid: string, phoneNumber: string, config: AgentConfig, uuid: string) {
    this.uuid        = uuid;
  ...
  static async create(callSid: string, phoneNumber: string, config: AgentConfig, uuid: string = 'inbound'): Promise<Call> {
  ...
  async setWebsocket(ws: WebSocket, streamSid: string): Promise<void> {
    ...
    ws.on('message', (message: Buffer | string) => {
      const msg = JSON.parse(message.toString()) as { event: string; media?: { payload: string }; streamSid?: string };
      switch (msg.event) {
        case 'media':
  ...
  async processResponse(geminiResponse: string): Promise<void> {
    try {
      ...
      if (fedToTwilio.action === 'check_availability') {
        ...
        const cannedStream = this.voices.setupGoogleTTSStream();
        cannedStream.write({ input: { text: 'Give me one second to check the calendar for you.' } });
        cannedStream.end();
